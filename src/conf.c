...
{
	/*burcu- if per_listener_settings is true, get security_options for the current listener,
	else global security options
	*/
...
/* ###### conf__attempt_resolve #####
get_addrinfo fonksiyonunu kullanarak host name'nin resolve edilip edilmediğini kontrol ediyor */
static int conf__attempt_resolve(const char *host, const char *text, unsigned int log, const char *msg)
{
	/* Burcu - addrinfo structure açıklama: https://www.qnx.com/developers/docs/6.5.0SP1.update/com.qnx.doc.neutrino_lib_ref/a/addrinfo.html*/
	struct addrinfo gai_hints;
	struct addrinfo *gai_res;
	int rc;
	/* -Burcu in order to use memset(), we should include <stdio.h> and <string.h>.
memset(&gai_hints, 0, sizeof(struct addrinfo)); -> gai_hints'in adresinden başlayarak
addrinfo size'ı kadar 0 ile doldur. */
	memset(&gai_hints, 0, sizeof(struct addrinfo));
	gai_hints.ai_family = AF_UNSPEC;
	gai_hints.ai_socktype = SOCK_STREAM;
	gai_res = NULL;
	/*Burcu - https://www.qnx.com/developers/docs/6.5.0SP1.update/com.qnx.doc.neutrino_lib_ref/g/getaddrinfo.html
	int getaddrinfo( const char * nodename,
		const char * servname,
		const struct addrinfo * hints,
		struct addrinfo ** res );
	gai_hints: yaratacağı linked listin structure yapısı
	gai_res: yaratılan linked listin başını point eden pointer
	rc = 0 for success*/
...
/*### config__init_reload ###
- burcu: Sanırım mqtt default configuration değerlerini veriyor */
...
- burcu: Mevcut plugin (eklenti) paketlerini sıfırlıyor (free edip null yapıyor) */
* /
	static void config__cleanup_plugins(struct mosquitto__config *config)
{
	int i, j;
...
/*### config__cleanup ###
Burcu - mevcut configuationların hepsinin memory alanlarını free ediyor
...
/* ### print_usage ###
Burcu:  mosquitto serverı start ederken parametre olarak -h verilirse
veya bilinmeyen bir optiyon verilirse bu fonskiyon çağırılıyor*/
...
/*### config__parse_args###
-Burcu : mosquitto serverı start ederken verilen parameterleri parse ediyor*/
int config__parse_args(struct mosquitto__config *config, int argc, char *argv[])
{
	...
	for (i = 1; i < argc; i++)
	{
		...
		/*Burcu: strcmp() = string compare -> return 0 if strings are equal
		eğer paremetre -c ise bir sonraki arguman config dosya adı olacak. Dosya
		adını da db.config_file'a set ediyor*/
		{
			if (i < argc - 1)
			{
				...
				{ /*Burcu - sanırım config dosyasını okuyamıyorsa hata return ediyor*/
					return MOSQ_ERR_INVAL;
				}
			}
			else
		...
		/*Burcu: eğer paremetre -p ise bir sonraki arguman TCP port numarası oluyor.
		 */
		{
			if (i < argc - 1)
			{
...
/*###  config__copy ###
Burcu: configurationın parametrelerini src'den dest'e kopyalıyor */
...
/*### config__read ###
Burcu: configurationı  bir dosyadan okuyor ve devreye alıyor
reload true ise önce config_reload structure'ına yüklüyor sonra config'e kopyalıyor
reload false ise direkt config'e yüklüyor */
int config__read(struct mosquitto__config *config, bool reload)
{
	...
	/*Burcu: eğer local_only true değilse ve allow_anonymous set edilmediyse false set ediyor*/
	{
		if (config->per_listener_settings)
		{
			for (i = 0; i < config->listener_count; i++)
		...
		if ((*buf)[0] != '#' && (*buf)[0] != 10 && (*buf)[0] != 13)
		{
			...
			if (token)
			{
				if (!strcmp(token, "acl_file"))
				{
					...
					if (conf__parse_string(&token, "acl_file", &cur_security_options->acl_file, saveptr))
						return MOSQ_ERR_INVAL;
				}
				...
				else if (!strcmp(token, "auth_plugin") || !strcmp(token, "plugin"))
				{
					...
					if (conf__parse_string(&token, "auth_plugin", &cur_auth_plugin_config->path, saveptr))
						return MOSQ_ERR_INVAL;
				}
				else if (!strcmp(token, "auth_plugin_deny_special_chars"))
				{
					...
					if (conf__parse_bool(&token, "auth_plugin_deny_special_chars", &cur_auth_plugin_config->deny_special_chars, saveptr))
						return MOSQ_ERR_INVAL;
				}
				...
				else if (!strcmp(token, "bridge_max_packet_size"))
				{
					...
					if (reload)
						continue; /* Bridges not valid for reloading. */
					if (!cur_bridge)
					{
						log__printf(NULL, MOSQ_LOG_ERR, "Error: Invalid bridge configuration.");
						return MOSQ_ERR_INVAL;
				...
				else if (!strcmp(token, "dhparamfile"))
				{
					...
					if (reload)
						continue; /* Listeners not valid for reloading. */
					if (conf__parse_string(&token, "dhparamfile", &cur_listener->dhparamfile, saveptr))
						return MOSQ_ERR_INVAL;
#else
					log__printf(NULL, MOSQ_LOG_WARNING, "Warning: TLS support not available.");
#endif
				...
				{
					if ((&token, token, &config->log_timestamp, saveptr))
						return MOSQ_ERR_INVAL;
				}
				...
				else if (!strcmp(token, "password_file"))
				{
					...
					if (conf__parse_string(&token, "password_file", &cur_security_options->password_file, saveptr))
						return MOSQ_ERR_INVAL;
				}
				...
				{
#ifdef WITH_WEBSOCKETS
					if (conf__parse_int(&token, "websockets_headers_size", &tmp_int, saveptr))
						return MOSQ_ERR_INVAL;
					if (tmp_int < 0 || tmp_int > UINT16_MAX)
					{
...
/* ###config__read_file###
Burcu: config_read fonksiyonu içerisinde çağırılıyor
ve dosyadan okumak için config__read_file_core fonskiyonunu çağırıyor  */
...
/*### config__check ###
Burcu: auto_id_prefix olarak auto-'yi set ediyor*/
...
/*### conf__parse_bool ###
Burcu: saveptr'de boolean değeri varsa onu value variable'ına set ediyor */
static int conf__parse_bool(char **token, const char *name, bool *value, char *saveptr)
{
	*token = strtok_r(NULL, " ", &saveptr); /*boşluk öncesi ilk kelimeyi alıyor*/
	if (*token)
	{
...
/*### conf__parse_int ###
Burcu: saveptr'de int değeri varsa onu value variable'ına set ediyor */
static int conf__parse_int(char **token, const char *name, int *value, char *saveptr)
{
	*token = strtok_r(NULL, " ", &saveptr);
	if (*token)
...
/*### conf__parse_ssize_t ###
Burcu: saveptr'de long int değeri varsa onu value variable'ına set ediyor */
static int conf__parse_ssize_t(char **token, const char *name, ssize_t *value, char *saveptr)
{
	*token = strtok_r(NULL, " ", &saveptr);
	if (*token)
	{
		*value = atol(*token);
		/*Burcu: The C library function long int atol(const char *str) converts the string argument str to a long integer (type long int).*/
	}
	else
	{
		log__printf(NULL, MOSQ_LOG_ERR, "Error: Empty %s value in configuration.", name);
...
/*### conf__parse_string ###
Burcu: saveptr'de str değeri varsa onu value variable'ına set ediyor */
static int conf__parse_string(char **token, const char *name, char **value, char *saveptr)
{
	size_t tlen;
