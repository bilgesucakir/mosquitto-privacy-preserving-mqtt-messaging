...
/*### nibble_to_hex ###
Burcu: 4 biti hexadecimal bir rakama çeviriyor. Örneğin 1010 = A
...

{
	...
	else
	{
		...
		/*Burcu: A'nın hex değeri 10. 65+10-10 = 65 = A*/
	}
}

/*### client_id_gen ###
Burcu: Client ID'si olmayan clientlara client ID generate ediyor*/
static char *client_id_gen(uint16_t *idlen, const char *auto_id_prefix, uint16_t auto_id_prefix_len)
{
	char *client_id;
	uint8_t rnd[16]; /*Burcu: 16 tane 8 bitlik sayı dizisi*/
	int i;
	int pos;

	if (util__random_bytes(rnd, 16)) /*Burcu: 16 tane 8 bitlik sayı dizisine random olarak değer veriyor */
		return NULL;

	*idlen = (uint16_t)(auto_id_prefix_len + 36); /*Burcu: config tanımlarındaki auto id prefixinde
	36 byte ekleyerek gerçek clientID length'i buluyor*/
	...
	if (auto_id_prefix)
	{
		...
		/*Burcu: client_id'nin başına configte tanımlı olan auto_id_prefix'ini kopyalıyor*/
	}

	pos = 0;
	for (i = 0; i < 16; i++)
	{
		client_id[auto_id_prefix_len + pos + 0] = nibble_to_hex(rnd[i] & 0x0F);		   /*son 4 bitini alıyor*/
		client_id[auto_id_prefix_len + pos + 1] = nibble_to_hex((rnd[i] >> 4) & 0x0F); /*ilk 4 bitini alıyor*/
		pos += 2;
...
/*Burcu: Queue'da olan mesajların hala acl'de yetkisi var mı yok mu onu kontrol ediyor,
yoksa queue'dan siliyor*/
static void connection_check_acl(struct mosquitto *context, struct mosquitto_client_msg **head)
{
	...
	DL_FOREACH_SAFE((*head), msg_tail, tmp) /*Burcu: linked listi tarıyor ve sırayla
	elemanları msg_tail'e alıyor*/
	{
		...
		{
			/*Burcu: Acl'den access yetkisi olup olmadığını kontrol ediyor, yoksa
			mesajı linked listten siliyor*/
...
Burcu: Yeni bir connection geldiğinde, connect olan client ile ilgili connection tablosunda
bir connection bilgisi var mı diye kontrol ediyor (HASH_FIND). Eğer varsa eski connectionın (found_context)
bazı context bilgilerini current connection'ının context'ine kopyalıyor */
int connect__on_authorised(struct mosquitto *context, void *auth_data_out, uint16_t auth_data_out_len)
{
	...
	if (found_context)
	{
		...
		if (context->clean_start == false && found_context->session_expiry_interval > 0)
		{
			...
			{
				connect_ack |= 0x01; /*Burcu: |= is a bitwise or. Connect_ack'nın son bitini 1 yapıyor*/
			}

			if (found_context->msgs_in.inflight || found_context->msgs_in.queued || found_context->msgs_out.inflight || found_context->msgs_out.queued)
			{
			...
			for (i = 0; i < context->sub_count; i++)
			{
				if (context->subs[i])
				{
					...
					while (leaf)
					{
						...
						{
							/*Burcu: Leaf->context'in değeri yeni context olmalı*/
		...
		if ((found_context->protocol == mosq_p_mqtt5 && found_context->session_expiry_interval == 0) || (found_context->protocol != mosq_p_mqtt5 && found_context->clean_start == true) || (context->clean_start == true))
		{
			/*Burcu: persistent session değilse, eski connection'dan kalan will mesajını gönderiyor */
		...
		mosquitto__set_state(found_context, mosq_cs_duplicate); /*Burcu: eski connectionın context'inin state'ini değiştiriyor*/
	...
	{
		/*Burcu: Eğer keepallive değeri max_keepalive'dan büyükse veya 0 ise ve mqtt version 5 değilse,
		disconnect ediyor*/
	...
	mosquitto__set_state(context, mosq_cs_active);							   /*Burcu: state of the context of the connection
								is set to active */
	rc = send__connack(context, connect_ack, CONNACK_ACCEPTED, connack_props); /*Burcu: send connack*/
